...
class RetrieverDataset(Dataset):
    ...
    def __init__(
        ...
        # Filter duplicate query rows unless the caller explicitly wants them.
        # `is_repeated` is set by build_dataset.py when the same query_id appears
        # in multiple pipeline runs.
        ...
        # Restrict to instruction-augmented rows (rows where has_instruction=True).
        # This effectively halves the dataset size and focuses training on
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # --- Query ---
        # Use the pre-assembled `query` field when present (generated by build_dataset.py);
        # otherwise fall back to concatenating the query and instruction manually.
        if "query" in row:
             query = row["query"]
        else:
        ...
        # In the current schema, `negative_passages` contains only hard negatives;
        # instruction negatives are stored separately in `new_negatives`.
        hard_negs = [self._format_passage(n) for n in neg_list]

        # Instruction negatives are only used for instruction-augmented rows.
        ...
        # Fill the remaining negative slots from the hard-negative pool.
        ...
        # If still below the target count, allow mixing the two negative types.
        ...
        # Zero-pad the negatives list to ensure a uniform collatable batch size.
