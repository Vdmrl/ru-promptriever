...
def write_hf_repo_files(output_dir, type_name="Dataset"):
    ...
    with open(readme_path, "w", encoding="utf-8") as f:
        f.write(f"# Promptriever {type_name}\n\n")
        ...
        f.write("It additionally supports creating two separate datasets: a clean **Training** dataset (containing just `query`, `positive_passages`, `negative_passages`) ")
        f.write("and an extended **Research** dataset containing detailed flags (`only_instruction`, `is_repeated`, etc.) mirroring the original paper's artifacts.\n\n")
        ...
        f.write("- `--include_research_dataset`: Flag to simultaneously generate an extended research dataset (outputs to `[output_dir]_research`).\n")
...
def filter_df(df, subset):
    if subset == "training":
        return df[["query_id", "query", "positive_passages", "negative_passages", "has_instruction", "is_repeated"]]
    else:
        return df[["query_id", "query", "positive_passages", "negative_passages", 
                   "only_instruction", "only_query", "has_instruction", 
                   "new_negatives", "is_repeated"]]
...
def main():
    ...
    parser.add_argument(
        "--include_research_dataset",
        action="store_true",
        help="If set, also generates a research dataset with all metadata columns.",
    )
    parser.add_argument(
        "--raw",
        action="store_true",
    ...
    save_sharded_parquet(filter_df(train_df, "training"), args.output_dir, "train", args.chunk_size)
    save_sharded_parquet(filter_df(val_df, "training"), args.output_dir, "val", args.chunk_size)
    save_sharded_parquet(filter_df(test_df, "training"), args.output_dir, "test", args.chunk_size)
    write_hf_repo_files(args.output_dir, type_name="Training Dataset")
    ...
    if args.include_research_dataset:
        print("  -> Exporting Research Dataset")
        research_dir = args.output_dir + "_research"
        os.makedirs(research_dir, exist_ok=True)
        save_sharded_parquet(filter_df(train_df, "research"), research_dir, "train", args.chunk_size)
        save_sharded_parquet(filter_df(val_df, "research"), research_dir, "val", args.chunk_size)
        save_sharded_parquet(filter_df(test_df, "research"), research_dir, "test", args.chunk_size)
        write_hf_repo_files(research_dir, type_name="Research Dataset")
